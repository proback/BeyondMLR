...
tModelName <- c("fit_1a_binom", "fit_1a_quasi", "fit_1b_binom", "fit_1b_quasi", "",
                "Model Name", "fit_2a_binom", "fit_2a_quasi", "fit_2b_binom", "fit_2b_quasi") 
tBeta <- c("","","","","",
           "$\\beta_1$", "","","","")
tSEBeta <- c("","","","","",
           "SE $\\beta_1$", "","","","")
...
kable(scenarioSimTab, "html", booktabs=T, caption="Summary of simulations for Dams and Pups case study") %>%
...
tibble(x = 1:24, pi_1b) %>%
  ggplot() +
    ...
    geom_density(data = theoretical_pi, aes(x = p1), bw = 0.025, linetype = 3) +
...
\begin{eqnarray}
  &\mbox{95% CI for}&\; p/(1-p) &=& (0.830, 1.378) \\
  &\mbox{95% CI for}&\; p       &=& (0.454, 0.579)
\end{eqnarray}
...
\begin{eqnarray}
  &\mbox{95% CI for}&\; p/(1-p) &=& (0.841, 1.359) \\
  &\mbox{95% CI for}&\; p       &=& (0.457, 0.576)
\end{eqnarray}
...
\begin{eqnarray}
  &\mbox{95% CI for}&\; p/(1-p) &=& (1.014, 1.691) \\
  &\mbox{95% CI for}&\; p    &=& (0.504, 0.628)
\end{eqnarray}
...
\begin{eqnarray}
  &\mbox{95% CI for}&\; p/(1-p) &=& (0.673, 2.594) \\
  &\mbox{95% CI for}&\; p    &=& (0.402, 0.722)
\end{eqnarray}
...
hist1 <- ggplot() + 
  geom_histogram(data = scenario_2[1:6,], bins = 5, aes(x = pi_2b, y = ..density..)) + 
  coord_cartesian(xlim = c(0,1)) +
  geom_density(data = theoretical_pi, aes(x = p1), bw = 0.05, linetype = 3) +
...
hist2 <- ggplot() + 
  geom_histogram(data = scenario_2[7:12,], aes(x = pi_2b, y = ..density..), bins = 5) +
  coord_cartesian(xlim = c(0,1)) +
  geom_density(data = theoretical_pi, aes(x = p2), bw = 0.05, linetype = 3) +
  geom_vline(xintercept = p_2[2], color = "red", linetype = 2) +
  labs(title = "Dosage = 1mg", x = "Probability of Deformity")

hist3 <- ggplot() + 
  geom_histogram(data = scenario_2[13:18,], aes(x = pi_2b, y = ..density..), bins = 5) +
  coord_cartesian(xlim = c(0,1)) +
  geom_density(data = theoretical_pi, aes(x = p3), bw = 0.05, linetype = 3) +
  geom_vline(xintercept = p_2[3], color = "red", linetype = 2) +
  labs(title = "Dosage = 2mg", x = "Probability of Deformity")

hist4 <- ggplot() + 
  geom_histogram(data = scenario_2[19:24,], aes(x = pi_2b, y = ..density..), bins = 5) +
  coord_cartesian(xlim = c(0,1)) +
  geom_density(data = theoretical_pi, aes(x = p4), bw = 0.05, linetype = 3) +
...
scenario2Tab <- scenario_2 %>%
                  ...
                  as.data.frame()
colnames(scenario2Tab) <- c("Dosage","Mean p", "SD p","Mean Count", "SD Count",
                            "Mean p", "SD p","Mean Count", "SD Count")
kable(scenario2Tab, "html", booktabs = T, caption="Summary Statistics of Scenario 2 by Dose") %>%
    add_header_above(c(" " = 1, "Scenario 2a" = 4, "Scenario 2b" = 4))
```
...
treetubes_yr1 <- read_csv("data/treetube.csv") %>%
  ...
  rename(tubes = TUBEX, id = ID, transect = TRANSECT, species = SPECIES,
         height91 = `1991`, height90 = `1990`) %>%
  dplyr::select(id, transect, species, tubes, height91, height90, growth_yr1)
```

```{r treeTubeTab, echo=FALSE}
kable(treetubes_yr1[c(137:146),], "html", booktabs = T, caption="A sample of 10 trees and their growth from 1990 to 1991")
```
...
__Tree-to-tree variability within transects__ There is inherent variability in tree growth even when they are subject to the same transect and treatment effects. This variability remains unexplained in our model.
...
```{r, treeTubesLinear, echo=FALSE, comment=NA}
tube_linear <- lm(growth_yr1 ~ tubes, data=treetubes_yr1)
# print(summary(tube_linear), show.resids=FALSE)
tmp <- capture.output(summary(tube_linear))
cat(tmp[c(3, 8:12)], sep='\n')
```
...
```{r treeTubesMulti1, echo=FALSE, comment=NA}
tube_multi1 <- lmer(growth_yr1 ~ tubes + (1|transect), 
                    data = treetubes_yr1)
# print(summary(tube_multi1), correlation=FALSE, show.resids=FALSE)
tmp <- capture.output(summary(tube_multi1))
...
c. *Cockroaches!*  For a study of cockroach infestation, traps were set up in the kitchen, bathroom and bedroom in a random sample of 100 NYC apartments. The goal is to estimate cockroach infestation levels given tenant income and age of the building.
...
c. Do the possible values depend on $a$ or $b$?
d. What is a feature of the beta density when $a=b$?
e. What happens to the density when $a \neq b$?
f. How does the magnitude of $a$ or $b$ affect the density?
g. How does the difference between $a$ and $b$ affect the density?
...
k. How might you decide on values for $a$ and $b$ if you have run a preliminary experiment and gathered data on the number of dams with deformed pups?
