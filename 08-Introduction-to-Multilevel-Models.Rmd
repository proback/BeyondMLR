...
The estimated within-person variance $\hat{\sigma}^2$ decreased by 3.1\% (from 22.5 to 21.8) from the unconditional means model, implying that only 3.1\% of within-person variability in performance anxiety scores can be explained by performance type. This calculation is considered a __pseudo R-square__ \index{pseudo R-square} value:
...
4. __Replicate the Sadler and Miller paper__ Try to replicate Models 1 and 2 presented in Table 2 of @Miller2010.  We expect small differences in parameter estimates, since they use SAS (with an unstructured covariance structure) instead of R.  
