...
#Getting started-2
table(music$id)     # number of diary entries for each subject
table(music$diary)  # number of subjects with diary entry of a given number

keydata = music[,c(2,3,5,6,7,9,11,12,14,16,17)]
head(keydata)
dim(keydata)   # 497x11
...
music.lev2 = keydata[keydata$diary==2 | (keydata$id==42 & keydata$diary==3),
  c(1,7,8,9,10,11)]
dim(music.lev2)   # should be 37 rows and 6 columns (one per L2 variable)

# Add average across all performances for each subject for EDA plots
meanbysubj = by(music$na,music$id,mean,na.rm=T)
music.lev2 = data.frame(music.lev2,meanbysubj=as.numeric(meanbysubj))
...
table(music$perform_type)   # gives number of performances of each type
table(music$memory)
table(music$audience)
...
na.all <- ggplot(data=music,aes(x=na)) + 
  geom_histogram(binwidth = 2, fill = "white",color = "black") + theme.1 + 
  xlab("Negative Affect") + ylab("Frequency") + labs(title="(a)") + xlim(10,35)
na.mean <- ggplot(data=music.lev2,aes(x=meanbysubj)) + 
  geom_histogram(binwidth = 2, fill = "white", color = "black") + theme.1 + 
  xlab("Mean Negative Affect") + ylab("Frequency") + labs(title="(b)") + xlim(10,35)
...
#Code needed for making next graph
...
music$orch = ifelse(music$instrument=="orchestral instrument",1,0)
music$large = ifelse(music$perform_type=="Large Ensemble",1,0)
...
\[ \textstyle{LargeEns}_{j} = \left\{ \begin{array}{ll}
                                1 & \mbox{if `perf_type` = Large Ensemble} \\
                                0 & \mbox{if `perf_type` = Solo or Small Ensemble.}
                                        \end{array}
                                \right.  \]
...
music$students = ifelse(music$audience=="Student(s)",1,0)
music$juried = ifelse(music$audience=="Juried Recital",1,0)
music$public = ifelse(music$audience=="Public Performance",1,0)
music$solo = ifelse(music$perform_type=="Solo",1,0)
music$memory1 = ifelse(music$memory=="Memory",1,0)
music$female = ifelse(music$gender=="Female",1,0)
music$vocal = ifelse(music$instrument=="voice",1,0)

#music[music$id==22,c("id","diary","perform_type","audience","na","instrument")]
id22 = music[music$id==22,]   
dim(id22)
```
...
```{r, include=FALSE}
music.lev2$orch = ifelse(music.lev2$instrument=="orchestral instrument",1,0)
last.visit=as.numeric(by(music$diary,music$id,max))
lvcf.na=rep(0,37)
for(i in 1:37)  {
  lvcf.na[i]=music$na[music$id==music.lev2$id[i] & music$diary==last.visit[i]]  }
lvcf.model=lm(lvcf.na~music.lev2$orch)
summary(lvcf.model)

int <- by(music, music$id, function(data)
                  coefficients(lm(na ~ large, data = data))[[1]])
summary(int)   
music.lev2$int = int[1:37]
...
Y_{ij} & = & [\alpha_{0}+\alpha_{1}\textstyle{Orch}_{i}+\beta_{0}\textstyle{LargeEns}_{ij}+\beta_{1}\textstyle{Orch}_{i}\textstyle{LargeEns}_{ij}] \\
 & & \mbox{} + [u_{i}+v_{i}\textstyle{LargeEns}_{ij}+\epsilon_{ij}]
\end{eqnarray*}
...
```{r, include=FALSE}
#code needed for next couple graphs
...
Y_{ij} & = & [\alpha_{0}+\alpha_{1}\textstyle{Orch}_{i}+\alpha_{2}\textstyle{MPQnem}_{i}+\beta_{0}\textstyle{LargeEns}_{ij} \\
 & & \mbox{} + \beta_{1}\textstyle{Orch}_{i}\textstyle{LargeEns}_{ij}+\beta_{2}\textstyle{MPQnem}_{i}\textstyle{LargeEns}_{ij}] \\
 & & \mbox{} + [u_{i}+v_{i}\textstyle{LargeEns}_{ij}+\epsilon_{ij}]
\end{eqnarray*}
...
Y_{ij} & = & \alpha_{0}+\alpha_{2}\textstyle{MPQnem}_{i}+\beta_{0}\textstyle{LargeEns}_{ij} \\
 & & \mbox{} +\beta_{2}\textstyle{MPQnem}_{i}\textstyle{LargeEns}_{ij} \\
\textstyle{Orch}=1: & & \\
...
#mean(music$mpqnem)
music$cmpqnem = music$mpqnem - mean(music$mpqnem)
...
#Code needed for next graphs
# Simulation 2 (take 1) - OLS SEs larger however and MLM better.  Try again.
subject = c(rep(1,20),rep(2,20),rep(3,20),rep(4,20))
b00 = 25
b10 = -0.5
e0i = rep(rnorm(4,0,20),each=20)  # no diff if sd=5
lambda0 = b00 + e0i
lambda1 = b10
previj = c(1:20,1:20,1:20,1:20)
eij = rnorm(80,0,4)
yij = lambda0 + lambda1*previj + eij
simdata = data.frame(subject=subject,lambda0=lambda0,lambda1=lambda1,
  previj=previj,eij=eij,yij=yij)
...
# Simulation 2 (take 2) - use estimated parameters from Model C
#   - models on simulated data don't always match Model C results
...
# Simulation 2 (take 2) - run 1000 times and summarize results
library(MASS)
orch.sim = music$orch
large.sim = music$large
subj.sim = music$id
...
grid.arrange(b00dens,b01dens,b10dens,b11dens,ncol=2)
```
