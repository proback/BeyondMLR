...
#Getting started
chart.wide = read.csv("data/chart_wide_condense.csv") 
```

<caption>(\#tab:table1chp9) The first six observations in the wide data set for the Charter Schools case study.

```{r table1ch9, echo=FALSE, warning=TRUE}
table1chp9 <- head(chart.wide[2:11])
kable(table1chp9, format = "html") %>%
...
table3chp9 <- head(smallchart.long[,c(3,5,7,8,9,10)])
kable(table3chp9, caption= "The first six observations in the long data set for the Charter Schools case study; these lines correspond to the first two observations from the wide data set illustrated in Table (ref:caplontable3chp9).")
```
...
mean3yr = by(chart.long$MathAvgScore,chart.long$schoolid,mean,na.rm=T)
urban0 = ifelse(chart.wide$urban==1,"urban","rural")
charter0 = ifelse(chart.wide$charter==1,"charter","public non-charter")
chart.wide = data.frame(chart.wide,mean3yr=as.numeric(mean3yr),
  urban0=urban0,charter0=charter0)

wide.charter = chart.wide[chart.wide$charter==1,]    # 73x11
...
wide.public = chart.wide[chart.wide$charter==0,]  
public.samp = wide.public[samp,]
sampdata = rbind(wide.charter, public.samp)

# Just use numbers 1-146 as school ids
vars <- 1 : 146
sampdata.ind = cbind(vars,sampdata)
dim(sampdata.ind)
head(sampdata.ind)

sampdata.l=reshape(sampdata.ind,idvar="vars",
  varying=c("MathAvgScore.0", "MathAvgScore.1", "MathAvgScore.2"),
  timevar="year08", time=c(0,1,2),
  direction="long") 
dim(sampdata.l)
...
summary(chart.wide$mean3yr[chart.wide$charter==1])
summary(chart.wide$mean3yr[chart.wide$charter==0])

#summaries (urban vs. rural)
summary(chart.wide$mean3yr[chart.wide$urban==1])
summary(chart.wide$mean3yr[chart.wide$urban==0])
...
SchPctFree = chart.long$schPctfree*100
SchPctSped = chart.long$schPctsped*100
SchPctNonw = chart.long$schPctnonw*100
SchPctFree.wide = chart.wide$schPctfree*100
SchPctSped.wide = chart.wide$schPctsped*100
SchPctNonw.wide = chart.wide$schPctnonw*100
...
schname10 = strtrim(smallchart.long$schoolName,10)
```
...
```{r, lon-lat2, fig.align="center",out.width="60%",fig.cap=' Lattice plot by school of math scores over time with linear fit for the first 24 schools in the data set.',echo=FALSE, warning=FALSE}
ggplot(smallchart.long,aes(x=year08,y=MathAvgScore)) + theme.1 + 
...
```{r, include=FALSE, warning=FALSE}
##Spaghetti Plots
#change labels to Public and Charter
mf_labeller <- function(value){
  value <- as.character(value)
  value[value=="0"] <- "Public Non-Charter"
  value[value=="1"]   <- "Charter"
  return(value)
}

#get rid of NA data
newsampdata.l <- na.omit(sampdata.l)
```

```{r, lon-spag3, fig.align="center",out.width="60%",fig.cap='Spaghetti plot showing time trends for each school by school type, for a random sample of charter schools (left) and public non-charter schools (right), with overall fits using loess (bold).',echo=FALSE, warning=FALSE, message=FALSE}
...
```{r, include=FALSE, warning=FALSE}
newsampdata.l$splitup <- as.numeric(Hmisc::cut2(newsampdata.l$schPctfree,g=4))
mf_labeller2 <- function(var, value){
  value <- as.character(value)
  if (var=="splitup") { 
    value[value=="1"] <- "First Quartile"
    value[value=="2"]   <- "Second Quartile"
    value[value=="3"] <- "Third Quartile"
    value[value=="4"]   <- "Fourth Quartile"
  }
  return(value)
}

to_string <- as_labeller(c(`1`="First Quartile",`2`="Second Quartile", `3`="Third Quartile", `4`="Fourth Quartile"))
```

```{r,lon-spagmat1, fig.align="center",out.width="60%",fig.cap='Spaghetti plot showing time trends for each school by quartiles of percent free and reduced lunch, with loess fits.',echo=FALSE, warning=FALSE}
ggplot(newsampdata.l,aes(x=year08,y=MathAvgScore)) + 
  geom_line(aes(group=schoolid),color="dark grey") + 
  facet_grid(~splitup,labeller=to_string) +
  labs(x="Years since 2008",y="Math Scores") + 
  scale_x_continuous(limits=c(0,2), breaks=c(0,1,2)) +
...
```{r, include=FALSE, warning=FALSE}
#95% CI's for slope and intercept of 24 schools			
MathAvgScore.list=lmList(MathAvgScore~year08 | schoolid, 
  data=smallchart.long, na.action=na.exclude)

#set up modeling and create data frame
int=rep(NA,length(unique(smallchart.long$schoolid)))			
intlb=rep(NA,length(unique(smallchart.long$schoolid)))
intub=rep(NA,length(unique(smallchart.long$schoolid)))
slp=rep(NA,length(unique(smallchart.long$schoolid)))
slplb=rep(NA,length(unique(smallchart.long$schoolid)))
slpub=rep(NA,length(unique(smallchart.long$schoolid)))
schools = as.numeric(smallchart.long$schoolid)
make.conf.frame <- function(schools) {
  for (i in unique(schools)) {
    model1 = lm(MathAvgScore~year08, data=smallchart.long, 
      subset=(schools==i), na.action=na.exclude)
    b0andb1 = summary(model1)$coef[,1]  # store model slope and intercept
    b0b1se = summary(model1)$coef[,2]    # store SEs of slope and intercept
    tstar = qt(.975,model1$df)
    lb = b0andb1 - tstar*b0b1se
    ub = b0andb1 + tstar*b0b1se
    int[i] = b0andb1[1]
    intlb[i] = lb[1]
    intub[i] = ub[1]
    slp[i] = b0andb1[2]
    slplb[i] = lb[2]
    slpub[i] = ub[2]  }
  conf.frame <- data.frame(int,intlb,intub,slp,slplb,slpub)
  return(conf.frame)
}
conf.frame <- make.conf.frame(schools)

#for next graph
slope.ci <- ggplot(conf.frame,aes(y=int,x=1:24)) + geom_point() + theme.1 + 
  geom_errorbar(aes(ymin=intlb,ymax=intub)) + 
  coord_flip() + labs(y="Intercepts",x="Schools",title="(a)")
int.ci <- ggplot(conf.frame,aes(y=slp,x=1:24)) + geom_point() + theme.1 + 
  geom_errorbar(aes(ymin=slplb,ymax=slpub)) + 
  coord_flip() + labs(y="Slopes",x="Schools",title="(b)")
```

```{r, lon-cis1, fig.align="center",out.width="60%",fig.cap='Point estimates and 95% confidence intervals for (a) intercepts and (b) slopes by school, for the first 24 schools in the data set.',echo=FALSE, warning=FALSE}
grid.arrange(slope.ci,int.ci,ncol=2)
```

```{r, include=FALSE, warning=FALSE}
# summary stats for intercepts  					
int <- by(chart.long, chart.long$schoolid, function(data)
  summary(lm(MathAvgScore ~ year08, data=data, 
  na.action=na.exclude))$coefficients[1,1])
summary(int)
sd(int)

# summary stats for fitted rate of change
rate <- by(chart.long, chart.long$schoolid, function(data)
  coefficients(lm(MathAvgScore ~ year08, data=data))[[2]])
summary(rate)
sd(rate,na.rm=T)

# summary stats for R sq
rsq <- by(chart.long, chart.long$schoolid, function(data)
  summary(lm(MathAvgScore ~ year08, data=data, 
...
rate <- as.vector(rate)
rsq <- as.vector(rsq)
histmat1.frame <- data.frame(int,rate,rsq)
int.hist1 <- ggplot(histmat1.frame) + 
  geom_histogram(aes(x=int),binwidth=5,color="black",fill="white") + theme.1 + 
  labs(x="Intercepts",y="Frequency",title="(a)")
rate.hist1 <- ggplot(histmat1.frame) + 
  geom_histogram(aes(x=rate),binwidth=5,color="black",fill="white") + theme.1 + 
  labs(x="Slopes",y="Frequency",title="(b)")
rsq.hist1 <- ggplot(histmat1.frame) + 
  geom_histogram(aes(x=rsq),binwidth=0.1,color="black",fill="white") + theme.1 +
  labs(x="Rsquared values",y="Frequency",title="(c)")
#for graph after next graph
cor(int, rate, use="complete.obs")
```

```{r, lon-histmat1, fig.align="center",out.width="60%",fig.cap=' Histograms for (a) intercepts, (b) slopes, and (c) R-square values from fitted regression lines by school.',echo=FALSE, warning=FALSE}
lon.histmat1 <- grid.arrange(int.hist1,rate.hist1,rsq.hist1,ncol=2)
```

```{r, lon-scat5, fig.align="center",out.width="60%",fig.cap='Scatterplot showing the relationship between intercepts and slopes from fitted regression lines by school.',echo=FALSE, warning=FALSE}
# correlation between slopes and intercepts for subjects with slope
ggplot(data=histmat1.frame,aes(x=int,y=rate)) + 
...
```{r, include=FALSE, warning=FALSE}
chart.long$charter0 = ifelse(chart.long$charter==1,"charter","public non-charter")
year08.df <- chart.long[which(chart.long$year08==0),]
year08.box <- ggplot(year08.df) + 
  geom_boxplot(aes(x=factor(charter0),y=MathAvgScore)) + 
  theme.1 + coord_flip() + labs(x="School Type",y="Math Score in 2008",title="(a)")
year10.df <- chart.long[which(chart.long$year08==2),]
year10.box <- ggplot(year10.df) + 
  geom_boxplot(aes(x=factor(charter0),y=MathAvgScore)) + 
...
int.scat1 <- ggplot(chart.wide,aes(x=100*schPctfree,y=int)) + geom_point() + 
...
rate.scat1 <- ggplot(chart.wide,aes(x=100*schPctfree,y=rate)) + geom_point() + 
  theme.1 + labs(x="Percent Free and Reduced Lunch",y="Fitted Slopes",title="(c)") + 
  geom_smooth(se=FALSE,method="lm",color="black",size=.75)
```

```{r, lon-boxcatmat1,fig.align="center",out.width="60%",fig.cap='(a) Boxplot of percent free and reduced lunch by school type (charter vs. public non-charter), along with scatterplots of (b) intercepts and (c) slopes from fitted regression lines by school vs. percent free and reduced lunch.',echo=FALSE, warning=FALSE}
...
pctfreew <- chart.wide$schPctfree
cor(cbind(pctfreew,int,rate),use="pairwise.complete.obs")
medpctfree=median(chart.wide$schPctfree)
highpctfree = ifelse(chart.wide$schPctfree > medpctfree,"High Pct Free/Reduced Lunch",
  "Low Pct Free/Reduced Lunch")
chart.wide = data.frame(chart.wide,highpctfree=highpctfree)
...
###Error structure within schools {#lineartwostageerror}
...
score.nonm = chart.long[!is.na(chart.long$MathAvgScore),]
hgtm.lm = lm(MathAvgScore~year08, data=score.nonm)
score.nonm$lmres = resid(hgtm.lm)

hgtwm = reshape(score.nonm[,c("schoolid","lmres","year08")],
                direction="wide", v.names="lmres", timevar="year08", idvar="schoolid")
```
...
```{r 9verb2, echo=FALSE, comment=NA}
...
###Unconditional growth model {#modelb}
...
```{r, lon-scat3, fig.align="center",out.width="60%",fig.cap='Linear growth model for Norwood Central Middle School',echo=FALSE, warning=FALSE}
#lon-scat3.eps
Norwood=chart.long[7:9,]
model0 = lm(Norwood$MathAvgScore~Norwood$year08)
ggplot(Norwood, aes(x=Norwood$year08,y=Norwood$MathAvgScore)) + 
  theme.1 + geom_point() + scale_y_continuous(limits=c(654,661)) +
  geom_smooth(se=FALSE,method="lm",color="black",size=.75) +
  labs(x="Years since 2008",y="Math Score",title="Norwood Central") + 
  geom_segment(aes(x = Norwood$year08[1], y = Norwood$MathAvgScore[1], 
    xend = Norwood$year08[1], yend = model0$fitted.values[1]), linetype=2) +
  geom_segment(aes(x = Norwood$year08[2], y = Norwood$MathAvgScore[2], 
    xend = Norwood$year08[2], yend = model0$fitted.values[2]), linetype=2) +
  geom_segment(aes(x = Norwood$year08[3], y = Norwood$MathAvgScore[3], 
    xend = Norwood$year08[3], yend = model0$fitted.values[3]), linetype=2)
```
...
```{r 9verb3, echo=FALSE, comment=NA}
...
```{r 9verb4, echo=FALSE, comment=NA}
# Modeling quadratic time trend
chart.long$yearc = chart.long$year08 - 1
chart.long$yearc2 = chart.long$yearc^2
...
model.b2 <- lmer(MathAvgScore~ yearc + yearc2 + (1|schoolid), 
  REML=T, data=chart.long)  # better than model.b - signif pos sq term
# print(summary(model.b2), correlation=FALSE, show.resids=FALSE)
tmp <- capture.output(summary(model.b2))
cat(tmp[c(2, 10:22)], sep='\n')
cat(" AIC = ", AIC(model.b2), '\n', "BIC = ", BIC(model.b2))
```
...
```{r 9verb5, echo=FALSE, comment=NA}
# Simple formulation of piecewise linear model which works since
#   we only have 3 observations per school
chart.long$year0809 = ifelse(chart.long$year08==2,0,chart.long$year08)
chart.long$year0810 = ifelse(chart.long$year08==0,0,chart.long$year08-1)

# tiny bit better than model.b2 but same story:
#   slope of 0.2 in 0809 but 2.5-0.2=2.3 in 0910
model.b6 <- lmer(MathAvgScore~ year0809 + year0810 + (1|schoolid), 
  REML=T, data=chart.long)
# print(summary(model.b6), correlation=FALSE, show.resids=FALSE)
tmp <- capture.output(summary(model.b6))
cat(tmp[c(2, 10:22)], sep='\n')
cat(" AIC = ", AIC(model.b6), '\n', "BIC = ", BIC(model.b6), '\n')

#model.b7 <- lmer(MathAvgScore~ year0809 + year0810 +   
#  (year0809+year0810|schoolid), REML=T, data=chart.long)
...
```{r 9verb6, echo=FALSE, comment=NA}
...
# print(summary(model.c), correlation=FALSE, show.resids=FALSE)
tmp <- capture.output(summary(model.c))
cat(tmp[c(2, 10:24)], sep='\n')
cat(" AIC = ", AIC(model.c), '\n', "BIC = ", BIC(model.c), '\n')
```
...
```{r 9verb7, echo=FALSE, comment=NA}
#Model D2 (Introduce SchPctFree at level 2)
model.d2 <- lmer(MathAvgScore~ charter + SchPctFree + year08 + 
  charter:year08 + SchPctFree:year08 + (year08|schoolid),
  REML=T, data=chart.long)
# print(summary(model.d2), correlation=FALSE, show.resids=FALSE)
tmp <- capture.output(summary(model.d2))
cat(tmp[c(2:3, 11:27)], sep='\n')
cat(" AIC = ", AIC(model.d2), '\n', "BIC = ", BIC(model.d2), '\n')
```
...
###A potential final model with three Level Two covariates {#modelf9}
...
```{r 9verb8, echo=FALSE, comment=NA, message=FALSE}
model.f2 <- lmer(MathAvgScore ~ charter + urban + SchPctFree + 
  SchPctSped + charter:year08 + urban:year08 + 
  SchPctSped:year08 + year08 +
  (year08|schoolid), REML=T, data=chart.long)
# print(summary(model.f2), correlation=FALSE, show.resids=FALSE)
tmp <- capture.output(summary(model.f2))
cat(tmp[c(2:3, 11:30)], sep='\n')
cat(" AIC = ", AIC(model.f2), '\n', "BIC = ", BIC(model.f2), '\n')
```
...
When the focus of an analysis is on stochastic parameters (variance components) rather than fixed effects, parameter estimates are typically based on restricted maximum likelihood (REML) methods; model performance statistics then reflect only the stochastic portion of the model. Models with the same fixed effects but different covariance structures can be compared as usual---with AIC and BIC measures when models are not nested and with likelihood ratio tests when models are nested.  However, using a chi-square distribution to conduct a likelihood ratio test in these cases can often produce a conservative test, with p-values that are too large and not rejected enough (@Bryk2002; @Singer2003; @Faraway2005). In Section \@ref(threelevel-paraboot), we introduce the parametric bootstrap as a potentially better way of testing models nested in their random effects.
...
cat(" AIC = ", AIC(model.f2a), '\n', "BIC = ", BIC(model.f2a))
```

```{r 9verb11, output.lines=-(1:7), comment=NA}
...
4. Walker-Barnes and Mason summarize their analytic approach in the following way:
...
5. The table below shows a portion of Table 2: Results of Hierarchical Linear Modeling Analyses Modeling Gang Involvement from @Walker-Barnes2001.  Provide interpretations of significant coefficients in context.
...
colnames(table4chp9) <- c("Predictor","\u03b3", "SE")
kable(table4chp9, booktabs=T, caption="A portion of Table 2: Results of Hierarchical Linear Modeling Analyses Modeling Gang Involvement from Walker-Barnes and Mason (2001).  These columns focus on the parenting behavior of psychological control.")
```
...
7. What implications do the scatterplots in Figure \@ref(fig:lon-boxcatmat1) (b) and (c) have for multilevel modeling?  What implications does the boxplot in Figure \@ref(fig:lon-boxcatmat1) (a) have?
...
12. In Section \@ref(modelb), why don't we examine the pseudo $R^2$ value for Level Two?
...
15. In Section \@ref(modeld), why do we look at a 10 percent increase in the percentage of students receiving free and reduced lunch when interpreting $\hat{\alpha}_{2}$?
...
1. __Teen Alcohol Use.__ @Curran1997 collected data on 82 adolescents at three time points starting at age 14 to assess factors that affect teen drinking behavior.  Key variables in the data set `alcohol.csv` (accessed via @Singer2003) are as follows:
    ...
    - do trajectories of alcohol use differ by peer alcohol use? </ul>

    1. Identify Level One and Level Two predictors.
    2. Perform a quick EDA.  What can you say about the shape of `alcuse`, and the relationship between `alcuse` and `coa`, `male`, and `peer`?  Appeal to plots and summary statistics in making your statements.
    3. Generate a plot as in Figure \@ref(fig:lon-lat1) with alcohol use over time for all 82 subjects.  Comment.
    4. Generate three spaghetti plots with loess fits similar to Figure \@ref(fig:lon-spag3) (one for `coa`, one for` male`, and one after creating a binary variable from `peer`).  Comment on what you can conclude from each plot.
    5. Fit a linear trend to the data from each of the 82 subjects using `age` as the time variable.  Generate histograms as in Figure \@ref(fig:lon-histmat1) showing the results of these 82 linear regression lines, and generate pairs of boxplots as in Figure \@ref(fig:lon-histmat1) for `coa` and `male`.  No commentary necessary.  [Hint: to produce Figure \@ref(fig:lon-histmat1), you will need a data frame with one observation per subject.]
    6. Repeat 5 using centered age (age14=age-14) as the time variable.  Also generate a pair of scatterplots as in Figure \@ref(fig:lon-boxcatmat1) for peer alcohol use.  Comment on trends you observe in these plots.  [Hint: after forming age14, append it to your current data frame.]
    7. Discuss similarities and differences between (5) and (6).  Why does using `age14` as the time variable make more sense in this example?
    8. (Model A) Run an unconditional means model.  Report and interpret the intraclass correlation coefficient.
    9. (Model B) Run an unconditional growth model with `age14` as the time variable at Level One.  Report and interpret estimated fixed effects, using proper notation.  Also report and interpret a pseudo-Rsquare value.
    10. (Model C) Build upon the unconditional growth model by adding the effects of having an alcoholic parent and peer alcohol use in both Level Two equations.  Report and interpret all estimated fixed effects, using proper notation.
    11. (Model D) Remove the child of an alcoholic indicator variable as a predictor of slope in Model C (it will still be a predictor of intercept).  Write out Model D as both a two-level and a composite model using proper notation (including error distributions); how many parameters (fixed effects and variance components) must be estimated?  Compare Model D to Model C using an appropriate method and state a conclusion.
...
2. __Completion Rates at US Colleges.__ Education researchers wonder which factors most affect the completion rates at US colleges.  Using the IPEDS database containing data from 1310 institutions over the years 2002-2009 [@IPEDS], the following variables were assembled in `colleges.csv`:  
    ...
    Level 2 predictors:  
    - `faculty` = mean number of full-time faculty per 100 students in between 2002-2009
    - `tuition` = mean yearly tuition in between 2002 and 2009 (in \$1000s) </ul>
