...
\item
  ...
  \item
    Interpret (in context) the LLSR estimates for \(\beta_3\) and \(\beta_5\) in Equation \eqref{eq:model0}.\\
  \end{enumerate}
\item
...
\item
  ...
  \item
    Fit a preliminary model with \texttt{MTL} as the response and \texttt{sitting} as the sole explanatory variable. Are LLSR conditions satisfied?
  \item
...
\section{\texorpdfstring{Linear Least Squares \index{linear least squares regression (LLSR)} vs.~Poisson Regression \index{Poisson regression}}{Linear Least Squares  vs.~Poisson Regression }}\label{linear-least-squares-vs.-poisson-regression}}
...
\item
  ...
  \end{enumerate}
\item
  \textbf{Campus crime.} The data set \texttt{campuscrime09.csv} contains the number of burglaries reported at a collection of 47 U.S. public universities with over 10,000 students in the year 2009. In addition, covariates are included which may explain differences in crime rates, including total number of students, percentage of men, average SAT and ACT test scores, and tuition.
...
Much like ordinary least squares (OLS), using \textbf{logistic regression} \index{logistic regression} to make inferences requires model assumptions.
...
\section{\texorpdfstring{Linear Least Squares \index{linear least squares regression (LLSR)} vs.~Binomial Regression \index{binomial logistic regression}}{Linear Least Squares  vs.~Binomial Regression }}\label{linear-least-squares-vs.-binomial-regression}}
...
The estimated within-person variance \(\hat{\sigma}^2\) decreased by 3.1\% (from 22.5 to 21.8) from the unconditional means model, implying that only 3.1\% of within-person variability in performance anxiety scores can be explained by performance type. This calculation is considered a \textbf{pseudo R-squared} \index{pseudo R-squared} value:
...
\item
  ...
  \item
    \texttt{attHeterogeneity} = a measure of the variability of ideologies within the district. Higher scores imply more attitudinal heterogeneity among voters.
  \item
...
\caption{\label{tab:table2chp9}A frequency table of missing data patterns.}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:table4chp10}Correlates of collective efficacy from Table 3 of Sampson et al. (1997).}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{llll}
\toprule
...
\caption{\label{tab:table5chp10}Neighborhood correlates of perceived neighborhood violence from a portion of Table 4 from Sampson et al. (1997).}
\centering
\resizebox{\linewidth}{!}{
...
\hypertarget{guided-exercises-8}{%
