...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{phi <-}\StringTok{ }\KeywordTok{sum}\NormalTok{(}\KeywordTok{resid}\NormalTok{(modeli, }\DataTypeTok{type=}\StringTok{'pearson'}\NormalTok{)}\OperatorTok{^}\DecValTok{2}\NormalTok{) }\OperatorTok{/}\StringTok{ }\NormalTok{modeli}\OperatorTok{$}\NormalTok{df.residual}
\NormalTok{drop.in.dev <-}\StringTok{ }\NormalTok{modeltr}\OperatorTok{$}\NormalTok{deviance }\OperatorTok{-}\StringTok{ }\NormalTok{modeli}\OperatorTok{$}\NormalTok{deviance}
\NormalTok{diff.in.df <-}\StringTok{ }\NormalTok{modeltr}\OperatorTok{$}\NormalTok{df.residual }\OperatorTok{-}\StringTok{ }\NormalTok{modeli}\OperatorTok{$}\NormalTok{df.residual}
\NormalTok{Fstat <-}\StringTok{ }\NormalTok{drop.in.dev }\OperatorTok{/}\StringTok{ }\KeywordTok{summary}\NormalTok{(modeliq)}\OperatorTok{$}\NormalTok{dispersion}
\NormalTok{Fstat}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{1}\OperatorTok{-}\KeywordTok{pf}\NormalTok{(Fstat, diff.in.df, modeli}\OperatorTok{$}\NormalTok{df.residual)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{gof.ts =}\StringTok{ }\NormalTok{pois.m1}\OperatorTok{$}\NormalTok{deviance}
\NormalTok{gof.pvalue =}\StringTok{ }\DecValTok{1} \OperatorTok{-}\StringTok{ }\KeywordTok{pchisq}\NormalTok{(gof.ts, pois.m1}\OperatorTok{$}\NormalTok{df.residual)}
...
\label{eq:binreg}
 \end{equation}
Models of this form are referred to as \textbf{binomial regression models},\index{binomial regression models} or more generally as \textbf{logistic regression models}. \index{logistic regression models} Here we provide intuition for using and interpreting logistic regression models, and then in the short optional section that follows, we present rationale for these models using GLM theory.
...
\NormalTok{mtemp1 =}\StringTok{ }\KeywordTok{c}\NormalTok{(mtemp1,}\KeywordTok{rep}\NormalTok{(moth}\OperatorTok{$}\NormalTok{dark[}\DecValTok{1}\NormalTok{],moth}\OperatorTok{$}\NormalTok{PLACED[}\DecValTok{1}\NormalTok{]}\OperatorTok{-}\NormalTok{moth}\OperatorTok{$}\NormalTok{REMOVED[}\DecValTok{1}\NormalTok{]))}
\NormalTok{dtemp1 =}\StringTok{ }\KeywordTok{c}\NormalTok{(dtemp1,}\KeywordTok{rep}\NormalTok{(moth}\OperatorTok{$}\NormalTok{DISTANCE[}\DecValTok{1}\NormalTok{],moth}\OperatorTok{$}\NormalTok{PLACED[}\DecValTok{1}\NormalTok{]}\OperatorTok{-}\NormalTok{moth}\OperatorTok{$}\NormalTok{REMOVED[}\DecValTok{1}\NormalTok{]))}
...
\NormalTok{  mtemp1 =}\StringTok{ }\KeywordTok{c}\NormalTok{(mtemp1,}\KeywordTok{rep}\NormalTok{(moth}\OperatorTok{$}\NormalTok{dark[i],moth}\OperatorTok{$}\NormalTok{PLACED[i]}\OperatorTok{-}\NormalTok{moth}\OperatorTok{$}\NormalTok{REMOVED[i]))}
\NormalTok{  dtemp1 =}\StringTok{ }\KeywordTok{c}\NormalTok{(dtemp1,}\KeywordTok{rep}\NormalTok{(moth}\OperatorTok{$}\NormalTok{DISTANCE[i],moth}\OperatorTok{$}\NormalTok{PLACED[i]}\OperatorTok{-}\NormalTok{moth}\OperatorTok{$}\NormalTok{REMOVED[i]))}
...
{\centering \includegraphics[width=0.6\linewidth]{bookdown-bysh_files/figure-latex/paraboot-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{anova}\NormalTok{(modelcl0,modelcl)   }\CommentTok{# go with Model C.0 (fewer varcomps)}

\CommentTok{# run bootstrapAnova function = parametric bootstrap code for lme4-models}
\CommentTok{#   http://stats.stackexchange.com/questions/4858/}
\CommentTok{#     longitudinal-data-analysis-by-multilevel-modeling-in-r/4870#4870}
\CommentTok{#   Posted on Nov 24, 2010, by Fabian Scheipl}

\CommentTok{#m0 is the lmer model under the null hypothesis (i.e. the smaller model)}
\CommentTok{#mA is the lmer model under the alternative}

\NormalTok{bootstrapAnova <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(mA, m0, }\DataTypeTok{B=}\DecValTok{100}\NormalTok{)\{}
\NormalTok{     oneBootstrap <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(m0, mA)\{}
\NormalTok{         d <-}\StringTok{ }\KeywordTok{drop}\NormalTok{(}\KeywordTok{simulate}\NormalTok{(m0))}
\NormalTok{         m2 <-}\KeywordTok{refit}\NormalTok{(mA, }\DataTypeTok{newresp=}\NormalTok{d)}
\NormalTok{         m1 <-}\KeywordTok{refit}\NormalTok{(m0, }\DataTypeTok{newresp=}\NormalTok{d)}
         \KeywordTok{return}\NormalTok{(}\KeywordTok{anova}\NormalTok{(m2,m1)}\OperatorTok{$}\NormalTok{Chisq[}\DecValTok{2}\NormalTok{])}
\NormalTok{     \}}
\NormalTok{     nulldist <-}\StringTok{ }\KeywordTok{replicate}\NormalTok{(B, }\KeywordTok{oneBootstrap}\NormalTok{(m0, mA))}
\NormalTok{     ret <-}\StringTok{ }\KeywordTok{anova}\NormalTok{(mA, m0)}
\NormalTok{     ret}\OperatorTok{$}\StringTok{"Pr(>Chisq)"}\NormalTok{[}\DecValTok{2}\NormalTok{] <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(ret}\OperatorTok{$}\NormalTok{Chisq[}\DecValTok{2}\NormalTok{] }\OperatorTok{<}\StringTok{ }\NormalTok{nulldist)}
     \KeywordTok{names}\NormalTok{(ret)[}\DecValTok{7}\NormalTok{] <-}\StringTok{ "Pr_boot(>Chisq)"}
     \KeywordTok{attr}\NormalTok{(ret, }\StringTok{"heading"}\NormalTok{) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{attr}\NormalTok{(ret, }\StringTok{"heading"}\NormalTok{)[}\DecValTok{1}\NormalTok{],}
          \KeywordTok{paste}\NormalTok{(}\StringTok{"Parametric bootstrap with"}\NormalTok{, B,}\StringTok{"samples."}\NormalTok{),}
          \KeywordTok{attr}\NormalTok{(ret, }\StringTok{"heading"}\NormalTok{)[}\OperatorTok{-}\DecValTok{1}\NormalTok{])}
     \KeywordTok{attr}\NormalTok{(ret, }\StringTok{"nulldist"}\NormalTok{) <-}\StringTok{ }\NormalTok{nulldist}
...
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.39\columnwidth}\centering
Neighborhood-level predictors\strut
\end{minipage} & \begin{minipage}[t]{0.17\columnwidth}\centering
\strut
\end{minipage} & \begin{minipage}[t]{0.11\columnwidth}\centering
\strut
...
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.39\columnwidth}\centering
Concentrated disadvantage\strut
\end{minipage} & \begin{minipage}[t]{0.17\columnwidth}\centering
-0.172\strut
\end{minipage} & \begin{minipage}[t]{0.11\columnwidth}\centering
0.016\strut
...
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.39\columnwidth}\centering
Percent of variance explained\strut
\end{minipage} & \begin{minipage}[t]{0.17\columnwidth}\centering
\strut
\end{minipage} & \begin{minipage}[t]{0.11\columnwidth}\centering
\strut
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Model F - final model?}
\NormalTok{model.f=}\KeywordTok{glmer}\NormalTok{(foul.home}\OperatorTok{~}\NormalTok{foul.diff}\OperatorTok{+}\NormalTok{score.diff}\OperatorTok{+}\NormalTok{lead.home}\OperatorTok{+}\NormalTok{time}\OperatorTok{+}\NormalTok{offensive}\OperatorTok{+}
\StringTok{  }\NormalTok{personal}\OperatorTok{+}\NormalTok{foul.diff}\OperatorTok{:}\NormalTok{offensive}\OperatorTok{+}\NormalTok{foul.diff}\OperatorTok{:}\NormalTok{personal}\OperatorTok{+}\NormalTok{foul.diff}\OperatorTok{:}\NormalTok{time}\OperatorTok{+}
\StringTok{  }\NormalTok{lead.home}\OperatorTok{:}\NormalTok{time}\OperatorTok{+}\NormalTok{(}\DecValTok{1}\OperatorTok{|}\NormalTok{game)}\OperatorTok{+}\NormalTok{(}\DecValTok{1}\OperatorTok{|}\NormalTok{hometeam)}\OperatorTok{+}\NormalTok{(}\DecValTok{1}\OperatorTok{|}\NormalTok{visitor),}\DataTypeTok{family=}\NormalTok{binomial,}
  \DataTypeTok{data=}\NormalTok{refdata)}
\KeywordTok{summary}\NormalTok{(model.f)}
\KeywordTok{exp}\NormalTok{(}\KeywordTok{fixef}\NormalTok{(model.f))}
...
\KeywordTok{hist}\NormalTok{(re.int,}\DataTypeTok{xlab=}\StringTok{"Random Effect"}\NormalTok{,}\DataTypeTok{main=}\StringTok{"Random Effects for Game"}\NormalTok{)}
\NormalTok{Home.re=}\KeywordTok{ranef}\NormalTok{(model.f)}\OperatorTok{$}\StringTok{`}\DataTypeTok{hometeam}\StringTok{`}\NormalTok{[[}\StringTok{"(Intercept)"}\NormalTok{]]}
\KeywordTok{hist}\NormalTok{(Home.re,}\DataTypeTok{xlab=}\StringTok{"Random Effect"}\NormalTok{,}\DataTypeTok{main=}\StringTok{"Random Effects for Home Team"}\NormalTok{)}
\NormalTok{Visiting.re=}\KeywordTok{ranef}\NormalTok{(model.f)}\OperatorTok{$}\StringTok{`}\DataTypeTok{visitor}\StringTok{`}\NormalTok{[[}\StringTok{"(Intercept)"}\NormalTok{]]}
\KeywordTok{hist}\NormalTok{(Visiting.re,}\DataTypeTok{xlab=}\StringTok{"Random Effect"}\NormalTok{,}
  \DataTypeTok{main=}\StringTok{"Random Effects for the Visiting Team"}\NormalTok{,}\DataTypeTok{xlim=}\KeywordTok{c}\NormalTok{(}\OperatorTok{-}\FloatTok{0.5}\NormalTok{,}\FloatTok{0.5}\NormalTok{))}
...
\NormalTok{ranef1=}\KeywordTok{dotplot}\NormalTok{(}\KeywordTok{ranef}\NormalTok{(model.f, }\DataTypeTok{postVar =} \OtherTok{TRUE}\NormalTok{), }\DataTypeTok{strip =} \OtherTok{FALSE}\NormalTok{)}
...
\label{tab:table4chp11} Adjusted rate ratios for individual-level variables from the multilevel Poisson regression model with random intercept for area from Table 2 in \citet{Randall2014}.
...
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.33\columnwidth}\raggedright
5 most disadvantaged\strut
\end{minipage} & \begin{minipage}[t]{0.14\columnwidth}\centering
1.7\strut
\end{minipage} & \begin{minipage}[t]{0.20\columnwidth}\centering
\textbf{1.52-1.91}\strut
\end{minipage} & \begin{minipage}[t]{0.12\columnwidth}\centering
\strut
\end{minipage}\tabularnewline
\bottomrule
