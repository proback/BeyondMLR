...
```{r, logistic1, fig.align="center",out.width="60%", fig.cap='An attempt to fit a linear regression model to a binary response variable.',echo=FALSE, warning=FALSE, message=FALSE}
...
```{r, lik1, fig.align="center",out.width="60%", fig.cap='Likehood function for GBB.',echo=FALSE, warning=FALSE, message=FALSE}
...
BBGGGplot<- ggplot(data=BBGGGGdf,aes(x=pb, y=lik2)) + 
  geom_line(color="blue", size=2) + 
  xlab("possible values of pb") + ylab("Likelihood") + 
  labs(title="Likelihood function for 2 families (GBB abd GGG)") +
  geom_vline(xintercept = .333) 
```

```{r, lik2, fig.align="center",out.width="60%", fig.cap='Likelihood function for the data of 2 families (GBB and GGG). The solid line is at the MLE,  ${p}_B=1/3$',echo=FALSE, warning=FALSE, message=FALSE}
BBGGGplot
```
...
kable(table2chp2, booktabs=T,caption="The likelihood factors for the hypothetical data set of n=50 children.")
```
...
```{r, lik4, fig.align="center",out.width="60%", fig.cap='Likelihood and log-likelihood functions for 50 children (30 boys and 20 girls) and for 1000 children (600 boys and 400 girls).',echo=FALSE, warning=FALSE, message=FALSE}
...
kable(table3chp2, booktabs=T,caption="Family contributions to the likelihood for a Sex Conditional Model using a hypothetical data set  of n=50 children from 30 families.")
```
...
Knowing that it is easier to work with log-likelihoods, let's take the log of the likelihood we constructed Equation \@ref(eq:lik50).
...
kable(table5chp2, booktabs=T, caption="Number of families and children in families with given composition. Sex ratio and proportion males are given by family size.")
```
...
kable(table6chp2, booktabs=T, caption="Proportion of families with all the same sex by number of children in the family. Note that 1-child families are all homogeneous with respect to sex so we look at 2- and 3- child families.")
```
...
kable(table7chp2,booktabs=T, caption="Proportion of families with only one boy who is born last.")
```
...
kable(table8chp2,booktabs=T, caption="Likelihood contributions for families in Sex Unconditional and Sex Conditional Models.", format="html") %>%
  add_header_above(c(" "," "," ", "Sex Conditional Model"=3))
```
...
kable(table9chp2, booktabs=T,caption="Likelihood contributions for families in Model 3: Waiting for a boy.")
```
